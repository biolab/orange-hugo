...
The curve that grows rapidly but then flattens out -- is probably China, right? To check this, select it. It may be hard to click the curve, so select it by dragging a line across it. Then, connect the Line Plot widget to Data Table, and it will show the data for this curve. You'll learn that this is, of course, the Hubei province – where it all started.
...
The population of Liechtenstein in millions with one decimal is 0.0; similar for Palau. To remove countries with unknown or zero population, we continue with Select Rows, where we set the condition "Total Population (millions) 2015 is greater than 0".

{{% sidework src="/blog_img/2020/2020-04-02-workflow-02-03.png" %}}
Now for some constructive business. To get the number of cases on some particular day, we use the Feature Constructor widget, where we can input a formula to compute new data columns. In "New" we select Numeric, type `cases per million` as the name of the new column, Select Feature "3_29_20" (or whichever date we'd like), add `/`, and then select "Total_Population\_\_millions\_\_2015". 
...
Things got crowded, so perhaps we can remove some columns. We add Select Columns, open it, and remove all columns we don't need right now. We can just select all features (Ctrl-A or Cmd-A) and drag them to the left, and then drag "cases per million" back to features.

Finally, add and connect a Data Table.

{{% figure src="/blog_img/2020/2020-04-02-workflow-1-day.png" %}}
...
{{% figure src="/blog_img/2020/2020-04-02-1-day-table.png" %}}
...
{{% sidework src="/blog_img/2020/2020-04-02-workflow-02-06.png" %}}
