...
For classification, when predicting the class value with [Predictions](/widget-catalog/evaluation/predictions), the widget will return relative frequencies of the classes in the training set. When there are two or more majority classes, the classifier chooses the predicted class randomly, but always returns the same class for a particular example.
...
![](/images/model/Constant-stamped.png)
...
In a typical classification example, we would use this widget to compare the scores of other learning algorithms (such as kNN) with the default scores. Use *iris* dataset and connect it to [Test & Score](/widget-catalog/evaluation/testandscore). Then connect **Constant** and [kNN](/widget-catalog/model/knn) to [Test & Score](/widget-catalog/evaluation/testandscore) and observe how well [kNN](/widget-catalog/model/knn) performs against a constant baseline.

![](/images/model/Constant-classification.png)

For regression, we use **Constant** to construct a predictor in [Predictions](/widget-catalog/evaluation/predictions). We used the *housing* dataset. In **Predictions**, you can see that *Mean Learner* returns one (mean) value for all instances.

![](/images/model/Constant-regression.png)
